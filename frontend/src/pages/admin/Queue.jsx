    ...
    <li className="flex items-start gap-3 py-3">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex flex-wrap items-baseline gap-x-2">
          ...
          <code className="text-xs text-content-subtle">#{job.id}</code>
        </div>
        {job.timestamp && (
          <p className="mt-0.5 text-xs text-content-muted">Queued {formatDateTime(job.timestamp)}</p>
        )}
        {detail}
      </div>
    </li>
      ...
      <div className="mt-6 grid grid-cols-1 gap-5 lg:grid-cols-2">
        <JobList
          ...
          renderDetail={(j) => (
            <>
              {j.failedReason && (
                <p className="mt-1.5 rounded-lg bg-danger-soft px-2.5 py-1.5 text-xs text-danger-text">
                  {j.failedReason}
                </p>
              )}
              {j.attemptsMade != null && (
        ...
        <JobList
          ...
          renderDetail={(j) =>
            j.delayUntil && (
              <p className="mt-1 text-xs text-content-muted">Runs at {formatDateTime(j.delayUntil)}</p>
            )
          }
        />
        ...
        <Panel title="How this works">
          <ol className="space-y-2.5 text-sm text-content-secondary">
            <li className="flex gap-2.5">
              <span className="grid h-5 w-5 shrink-0 place-items-center rounded-full bg-surface-hover text-xs font-semibold tabular-nums text-content-secondary">1</span>
              An order is placed or a payment settles.
            </li>
            <li className="flex gap-2.5">
              <span className="grid h-5 w-5 shrink-0 place-items-center rounded-full bg-surface-hover text-xs font-semibold tabular-nums text-content-secondary">2</span>
              A job is added to the BullMQ queue, held in Redis.
            </li>
            <li className="flex gap-2.5">
              <span className="grid h-5 w-5 shrink-0 place-items-center rounded-full bg-surface-hover text-xs font-semibold tabular-nums text-content-secondary">3</span>
              The notification worker picks it up and sends the push notification.
            </li>
            <li className="flex gap-2.5">
              <span className="grid h-5 w-5 shrink-0 place-items-center rounded-full bg-surface-hover text-xs font-semibold tabular-nums text-content-secondary">4</span>
